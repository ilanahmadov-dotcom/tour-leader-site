...
function fillStationSelectSafe() {
  ...
  for (var j = 0; j < location.stations.length; j++) {
    var opt = document.createElement("option"); opt.value = location.stations[j].id; opt.textContent = location.stations[j].name; stationSelect.appendChild(opt);
  }
  checkLiveCapacitySafe();
}
...
function enterLeaderSafe() {
  try {
    ...
    if (!groups[id]) groups[id] = { name: name, bigGroup: bigGroup, status: "not started", lastStatusTime: "", active: null, waitingFor: null, userCreated: true };
...
function loadLocalState() {
  try { return JSON.parse(localStorage.getItem("tourState") || "{}"); } catch (e) { return {}; }
}
function saveLocalState() {
  localStorage.setItem("tourState", JSON.stringify({ groups: groups, messages: messages }));
  localStorage.setItem("tourGroups", JSON.stringify(groups));
}
function normalizeGroups(data) {
  var merged = data || {};
  for (var id in merged) if (Object.prototype.hasOwnProperty.call(merged, id)) {
    var group = merged[id] || {}; group.name = group.name || id; group.bigGroup = group.bigGroup || (id.indexOf("-B-") >= 0 ? "B" : "A");
    group.status = group.status || "not started"; group.lastStatusTime = group.lastStatusTime || ""; group.active = normalizeActive(group.active); group.waitingFor = normalizeWaiting(group.waitingFor); merged[id] = group;
  }
  return merged;
}
function normalizeMessages(arr) { if (!arr || !arr.length) return []; return arr.slice(-80); }
function normalizeWaiting(waiting) { if (!waiting) return null; var st = getStationInfo(waiting.stationId || waiting.stationName); if (!st) return null; return { stationId: st.id, stationName: st.name, locationName: st.locationName, time: waiting.time || new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) }; }
...
function showDashboard(groupId) {
  ...
  updateCurrentDisplay(); checkLiveCapacitySafe(); renderOverviewSafe(); renderMessagesSafe();
...
function finishStationSafe() {
  ...
  var oldStation = groups[selectedGroupId].active ? groups[selectedGroupId].active.stationName : "station";
  groups[selectedGroupId].active = null; groups[selectedGroupId].status = "finished early"; groups[selectedGroupId].lastStatusTime = nowTime();
  sendMessageToWaitingForMyStation("free", "I finished at " + oldStation + ". You can enter now.");
  var synced = saveAllState(); updateCurrentDisplay(); renderOverviewSafe(); renderMessagesSafe(); alert(synced ? "Finished and synced." : "Finished only on this phone.");
}

function waitingForStationSafe() {
  if (!selectedGroupId || !groups[selectedGroupId]) { alert("Enter your name first."); return; }
  var station = getStationInfo($("stationSelect") ? $("stationSelect").value : ""); if (!station) return;
  groups[selectedGroupId].status = "waiting"; groups[selectedGroupId].waitingFor = { stationId: station.id, stationName: station.name, locationName: station.locationName, time: nowTime() }; groups[selectedGroupId].lastStatusTime = nowTime();
  var holders = getActiveGroupsAtStation(station.id).filter(function (item) { return item.id !== selectedGroupId; });
  if (holders.length === 0) addMessage("system", selectedGroupId, "No one is marked inside " + station.name + ". You may be able to enter.", "info", station.id);
  for (var i = 0; i < holders.length; i++) addMessage(selectedGroupId, holders[i].id, groups[selectedGroupId].name + " is waiting for " + station.name + ".", "waiting", station.id);
  var synced = saveAllState(); renderOverviewSafe(); renderMessagesSafe(); alert(synced ? "Waiting message sent." : "Waiting saved only on this phone.");
}

function delayedHereSafe() {
  if (!groups[selectedGroupId]) { alert("Enter your name first."); return; }
  groups[selectedGroupId].status = "delayed"; groups[selectedGroupId].lastStatusTime = nowTime();
  var sent = sendMessageToWaitingForMyStation("delayed", groups[selectedGroupId].name + " is delayed at this station.");
  var synced = saveAllState(); renderOverviewSafe(); renderMessagesSafe(); alert(sent ? (synced ? "Delay sent to waiting leaders." : "Delay saved only on this phone.") : "No waiting leaders found. Delay status saved.");
}
...
function setStatusSafe(status) { if (status === "waiting") return waitingForStationSafe(); if (status === "delayed") return generalDelayedSafe(); if (!groups[selectedGroupId]) return; groups[selectedGroupId].status = status; groups[selectedGroupId].lastStatusTime = nowTime(); saveAllState(); renderOverviewSafe(); }

function sendMessageToWaitingForMyStation(type, text) {
  if (!groups[selectedGroupId] || !groups[selectedGroupId].active) return 0;
  var stationId = groups[selectedGroupId].active.stationId, sent = 0;
  for (var id in groups) if (Object.prototype.hasOwnProperty.call(groups, id) && id !== selectedGroupId) {
    if (groups[id].waitingFor && groups[id].waitingFor.stationId === stationId) { addMessage(selectedGroupId, id, text, type, stationId); sent++; }
  }
  return sent;
}
...
function updateCurrentDisplay() {
  if (!selectedGroupId || !groups[selectedGroupId]) return; var active = groups[selectedGroupId].active;
  if (!active) { $("currentStation").textContent = groups[selectedGroupId].waitingFor ? "Waiting for: " + groups[selectedGroupId].waitingFor.stationName : "Not started"; $("timeLeft").textContent = "--:--"; $("currentWindow").textContent = "Choose a station and press Start now, or press Waiting."; $("nextStation").textContent = groups[selectedGroupId].waitingFor ? "Location: " + groups[selectedGroupId].waitingFor.locationName : "Location: -"; return; }
  var left = Math.max(0, Math.round((active.endMs - new Date().getTime()) / 1000)); $("currentStation").textContent = active.stationName; $("timeLeft").textContent = formatSeconds(left); $("currentWindow").textContent = active.duration + " minutes · started " + new Date(active.startMs).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }); $("nextStation").textContent = "Location: " + active.locationName;
  if (left === 0) { $("timeLeft").classList.add("alertDone"); maybeNotify("Station time finished", "Press Finish or choose the next station."); } else $("timeLeft").classList.remove("alertDone");
}

function checkLiveCapacitySafe() {
  var warning = $("capacityWarning"); if (!warning) return; var station = getStationInfo($("stationSelect") ? $("stationSelect").value : ""); warning.classList.add("hidden"); warning.textContent = ""; if (!station || station.capacity >= 9999) return;
  var activeGroups = getActiveGroupsAtStation(station.id).filter(function (item) { return item.id !== selectedGroupId; });
  if (activeGroups.length >= station.capacity) { warning.classList.remove("hidden"); warning.textContent = "Full now: " + activeGroups.map(function (item) { return item.group.name; }).join(", ") + " already at this station."; }
  else if (activeGroups.length > 0) { warning.classList.remove("hidden"); warning.textContent = "Currently there: " + activeGroups.map(function (item) { return item.group.name; }).join(", ") + ". Capacity: " + station.capacity + "."; }
}

function renderMessagesSafe() {
  var box = $("messagesBox"); if (!box || !selectedGroupId) return; box.innerHTML = ""; var mine = [];
  for (var i = messages.length - 1; i >= 0; i--) if (messages[i].toId === selectedGroupId) mine.push(messages[i]);
  if (!mine.length) { box.innerHTML = '<p class="hint">No messages.</p>'; return; }
  for (var j = 0; j < mine.length && j < 12; j++) { var m = mine[j]; var div = document.createElement("div"); div.className = "stationBox" + (m.type === "waiting" ? " over" : ""); div.innerHTML = '<div class="stationTitle"><span>' + escapeHtml(m.fromName || "Message") + '</span><span>' + escapeHtml(m.time || "") + '</span></div><p>' + escapeHtml(m.text) + '</p>'; box.appendChild(div); }
}
...
function renderOverviewSafe() {
  var container = $("stationsOverview"); if (!container) return; var byStation = {}, waitingByStation = {}; for (var i = 0; i < ALL_STATIONS.length; i++) { byStation[ALL_STATIONS[i].id] = []; waitingByStation[ALL_STATIONS[i].id] = []; } var idleList = [];
  for (var id in groups) if (Object.prototype.hasOwnProperty.call(groups, id)) { var group = groups[id]; if (group.active) { if (!byStation[group.active.stationId]) byStation[group.active.stationId] = []; byStation[group.active.stationId].push({ id: id, group: group }); } else if (group.waitingFor) { if (!waitingByStation[group.waitingFor.stationId]) waitingByStation[group.waitingFor.stationId] = []; waitingByStation[group.waitingFor.stationId].push({ id: id, group: group }); } else idleList.push({ id: id, group: group }); }
  container.innerHTML = "";
  for (var l = 0; l < LOCATIONS.length; l++) { var heading = document.createElement("h3"); heading.textContent = LOCATIONS[l].name; heading.dir = "rtl"; container.appendChild(heading);
    for (var st = 0; st < LOCATIONS[l].stations.length; st++) { var station = LOCATIONS[l].stations[st], list = byStation[station.id] || [], waits = waitingByStation[station.id] || [], isOver = station.capacity < 9999 && list.length > station.capacity, limitText = station.capacity >= 9999 ? "no limit" : "limit " + station.capacity, box = document.createElement("div");
      box.className = "stationBox" + (isOver ? " over" : ""); box.innerHTML = '<div class="stationTitle"><span dir="rtl">' + escapeHtml(station.name) + '</span><span>' + list.length + ' inside · ' + waits.length + ' waiting · ' + limitText + '</span></div>';
      for (var p = 0; p < list.length; p++) { var pill = document.createElement("span"), g = list[p].group, status = g.status || "active", left = g.active ? Math.max(0, Math.round((g.active.endMs - new Date().getTime()) / 1000)) : 0; pill.className = "groupPill" + (status === "delayed" ? " delayed" : status === "finished early" ? " good" : ""); pill.textContent = g.name + " (" + (g.bigGroup || "") + ") - inside - " + formatSeconds(left); box.appendChild(pill); }
      for (var w = 0; w < waits.length; w++) { var wp = document.createElement("span"), wg = waits[w].group; wp.className = "groupPill waiting"; wp.textContent = wg.name + " (" + (wg.bigGroup || "") + ") - waiting"; box.appendChild(wp); }
      container.appendChild(box);
    }}
  var idleBox = document.createElement("div"); idleBox.className = "stationBox"; idleBox.innerHTML = '<div class="stationTitle"><span>Not started / finished</span><span>' + idleList.length + ' leaders</span></div>'; for (var k = 0; k < idleList.length; k++) { var ipill = document.createElement("span"); ipill.className = "groupPill"; ipill.textContent = idleList[k].group.name + " (" + (idleList[k].group.bigGroup || "") + ") - " + (idleList[k].group.status || "not started"); idleBox.appendChild(ipill); } container.appendChild(idleBox);
}
...
function maybeNotify(title, body) { var key = title + body + new Date().toISOString().slice(0, 16); if (lastNotificationMinute === key) return; lastNotificationMinute = key; if ("Notification" in window && Notification.permission === "granted") new Notification(title, { body: body }); if (navigator.vibrate) navigator.vibrate([250, 120, 250]); }
