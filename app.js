...
var lastLocalEdit = Number(localStorage.getItem("lastLocalEdit") || "0");
var loadedRemoteOnce = false;
...
function boot() {
  try {
    ...
    renderOverviewSafe(); renderMessagesSafe(); renderPlanSafe(); loadFirebaseScripts();
  } catch (e) { setSyncStatus("Script error: " + e.message); alert("Site script error: " + e.message); }
}

function loadFirebaseScripts() {
  var appScript = document.createElement("script"); appScript.src = "https://www.gstatic.com/firebasejs/8.10.1/firebase-app.js";
  appScript.onload = function () { var fsScript = document.createElement("script"); fsScript.src = "https://www.gstatic.com/firebasejs/8.10.1/firebase-firestore.js"; fsScript.onload = initFirebaseSafe; fsScript.onerror = function () { setSyncStatus("Local mode only - Firebase script failed"); }; document.head.appendChild(fsScript); };
  appScript.onerror = function () { setSyncStatus("Local mode only - Firebase script failed"); }; document.head.appendChild(appScript);
}

function initFirebaseSafe() {
  try {
    firebase.initializeApp(firebaseConfig); db = firebase.firestore(); setSyncStatus("Connecting to Firebase...");
    db.collection("tour").doc("state").onSnapshot(function (snap) {
      setSyncStatus("Online sync enabled");
      var localSnapshot = { groups: groups, messages: messages };
      if (snap.exists) {
        var data = snap.data() || {};
        var remoteGroups = normalizeGroups(data.groups || {});
        var remoteMessages = normalizeMessages(data.messages || []);
        if (pendingSave && selectedGroupId && localSnapshot.groups[selectedGroupId]) {
          remoteGroups[selectedGroupId] = localSnapshot.groups[selectedGroupId];
          remoteMessages = mergeMessages(remoteMessages, localSnapshot.messages || []);
          groups = remoteGroups; messages = remoteMessages; saveLocalState(); pushPendingSave();
        } else {
          groups = remoteGroups; messages = remoteMessages; saveLocalState();
        }
      } else {
        pendingSave = true; localStorage.setItem("pendingSave", "true"); pushPendingSave();
      }
      loadedRemoteOnce = true;
      updateCurrentDisplay(); checkLiveCapacitySafe(); renderOverviewSafe(); renderMessagesSafe(); renderPlanSafe(); notifyNewMessagesForMe();
    }, function (err) { db = null; setSyncStatus("Firebase error: " + (err.code || err.message)); alert("Firebase cannot sync. Check Firestore rules/test mode."); });
  } catch (e) { db = null; setSyncStatus("Firebase error: " + e.message); }
}

function mergeMessages(a, b) {
  var seen = {}, out = [], i;
  for (i = 0; i < a.length; i++) { if (!seen[a[i].id]) { seen[a[i].id] = true; out.push(a[i]); } }
  for (i = 0; i < b.length; i++) { if (!seen[b[i].id]) { seen[b[i].id] = true; out.push(b[i]); } }
  out.sort(function(x,y){ return (x.ts||0) - (y.ts||0); }); return out.slice(-80);
}

function pushPendingSave() {
  if (!db) return false;
  db.collection("tour").doc("state").set({ groups: groups, messages: messages }, { merge: true }).then(function () { pendingSave = false; localStorage.setItem("pendingSave", "false"); setSyncStatus("Online sync enabled"); }).catch(function (err) { pendingSave = true; localStorage.setItem("pendingSave", "true"); setSyncStatus("Save failed: " + (err.code || err.message)); });
  return true;
}

function markDirty() { pendingSave = true; lastLocalEdit = new Date().getTime(); localStorage.setItem("pendingSave", "true"); localStorage.setItem("lastLocalEdit", String(lastLocalEdit)); }
...
function fillLocationSelectSafe() { var select = $("locationSelect"); if (!select) return; select.innerHTML = ""; for (var i = 0; i < LOCATIONS.length; i++) { var opt = document.createElement("option"); opt.value = LOCATIONS[i].id; opt.textContent = LOCATIONS[i].name; select.appendChild(opt); } fillStationSelectSafe(); }
function fillStationSelectSafe() { var locationSelect = $("locationSelect"), stationSelect = $("stationSelect"); if (!stationSelect) return; var locationId = locationSelect ? locationSelect.value : LOCATIONS[0].id; fillStationSelectElement(stationSelect, locationId, null); checkLiveCapacitySafe(); }
function fillStationSelectElement(select, locationId, selectedStationId) { select.innerHTML = ""; var location = LOCATIONS[0]; for (var i = 0; i < LOCATIONS.length; i++) if (LOCATIONS[i].id === locationId) location = LOCATIONS[i]; for (var j = 0; j < location.stations.length; j++) { var opt = document.createElement("option"); opt.value = location.stations[j].id; opt.textContent = location.stations[j].name; select.appendChild(opt); } if (selectedStationId) select.value = selectedStationId; }

function enterLeaderSafe() {
  try {
    var name = (( $("leaderNameInput") && $("leaderNameInput").value) || "").trim(); var bigGroup = (($("bigGroupSelect") && $("bigGroupSelect").value) || "A"); if (!name) { alert("Write your leader/team name first."); return; }
    var id = makeLeaderId(name, bigGroup); localStorage.setItem("leaderName", name); localStorage.setItem("bigGroup", bigGroup);
    ...
    groups[id].name = name; groups[id].bigGroup = bigGroup; groups[id].userCreated = true; selectedGroupId = id; localStorage.setItem("selectedGroupId", id);
    saveAllState(false); showDashboard(id);
  } catch (e) { alert("Enter failed: " + e.message); setSyncStatus("Enter error: " + e.message); }
}
function makeLeaderId(name, bigGroup) { var hash = 0, raw = String(bigGroup) + ":" + String(name).trim().toLowerCase(); for (var i = 0; i < raw.length; i++) { hash = ((hash << 5) - hash) + raw.charCodeAt(i); hash = hash | 0; } if (hash < 0) hash = -hash; return "leader-" + bigGroup + "-" + hash; }
...
function normalizeGroups(data) { var merged = data || {}; for (var id in merged) if (Object.prototype.hasOwnProperty.call(merged, id)) { var group = merged[id] || {}; group.name = group.name || id; group.bigGroup = group.bigGroup || (id.indexOf("-B-") >= 0 ? "B" : "A"); group.status = group.status || "not started"; group.lastStatusTime = group.lastStatusTime || ""; group.active = normalizeActive(group.active); group.waitingFor = normalizeWaiting(group.waitingFor); group.routePlan = normalizeRoutePlan(group.routePlan || []); group.planIndex = Number(group.planIndex || 0); merged[id] = group; } return merged; }
...
function normalizeActive(active) { if (!active) return null; var station = getStationInfo(active.stationId || active.station || active.stationName); if (!station) return null; var startMs = Number(active.startMs || active.startedAt || new Date().getTime()); var duration = Number(active.duration || active.durationMinutes || 15); return { locationId: station.locationId, locationName: station.locationName, stationId: station.id, stationName: station.name, startMs: startMs, duration: duration, endMs: startMs + duration * 60 * 1000 }; }
function normalizeRoutePlan(plan) { var out = []; for (var i = 0; i < plan.length; i++) { var st = getStationInfo(plan[i].stationId || plan[i].stationName); if (st) out.push({ locationId: st.locationId, stationId: st.id, stationName: st.name, duration: Number(plan[i].duration || 15) }); } return out; }
function saveAllState(showError) { markDirty(); saveLocalState(); if (!db) { setSyncStatus("Local changes pending sync"); return false; } pushPendingSave(); return true; }

function showDashboard(groupId) { selectedGroupId = groupId; localStorage.setItem("selectedGroupId", groupId); if ($("setupCard")) $("setupCard").classList.add("hidden"); if ($("dashboard")) $("dashboard").classList.remove("hidden"); if ($("groupName")) $("groupName").textContent = (groups[groupId].name || groupId) + " · Big Group " + (groups[groupId].bigGroup || ""); updateCurrentDisplay(); checkLiveCapacitySafe(); renderOverviewSafe(); renderMessagesSafe(); renderPlanSafe(); if (timerInterval) clearInterval(timerInterval); timerInterval = setInterval(function () { updateCurrentDisplay(); renderOverviewSafe(); }, 1000); }
...
function addPlanStepSafe(step) { if (!selectedGroupId || !groups[selectedGroupId]) { alert("Enter your name first."); return; } groups[selectedGroupId].routePlan = groups[selectedGroupId].routePlan || []; groups[selectedGroupId].routePlan.push(step || { locationId: "tzrif", stationId: "tzrif1", duration: 15 }); renderPlanSafe(); }
function renderPlanSafe() { var list = $("routePlanList"); if (!list || !selectedGroupId || !groups[selectedGroupId]) return; var plan = groups[selectedGroupId].routePlan || []; list.innerHTML = ""; if (!plan.length) addPlanStepSafe({ locationId: "tzrif", stationId: "tzrif1", duration: 15 }); plan = groups[selectedGroupId].routePlan || []; for (var i = 0; i < plan.length; i++) { var row = document.createElement("div"); row.className = "stop"; row.innerHTML = '<label>Step ' + (i + 1) + ' location<select class="planLocation" data-i="' + i + '" onchange="planLocationChangedSafe(this)"></select></label><label>Station<select class="planStation stationInput" data-i="' + i + '" onchange="updatePlanFromUI()"></select></label><label>Minutes<input class="planDuration" data-i="' + i + '" type="number" min="1" max="180" value="' + (plan[i].duration || 15) + '" oninput="updatePlanFromUI()" /></label><button class="ghost" onclick="removePlanStepSafe(' + i + ')">Remove</button>'; list.appendChild(row); var locSel = row.querySelector(".planLocation"), stSel = row.querySelector(".planStation"); fillPlanLocationSelect(locSel, plan[i].locationId || "tzrif"); fillStationSelectElement(stSel, locSel.value, plan[i].stationId); } renderPlanWarningsSafe(); }
...
function updatePlanFromUI() { if (!selectedGroupId || !groups[selectedGroupId]) return; var rows = document.querySelectorAll("#routePlanList .stop"), plan = []; for (var i = 0; i < rows.length; i++) { var loc = rows[i].querySelector(".planLocation").value, st = rows[i].querySelector(".planStation").value, dur = Number(rows[i].querySelector(".planDuration").value || 15), info = getStationInfo(st); if (info) plan.push({ locationId: loc, stationId: info.id, stationName: info.name, duration: dur }); } groups[selectedGroupId].routePlan = plan; renderPlanWarningsSafe(); }
...
function savePlanSafe() { updatePlanFromUI(); var conflicts = getPlanConflictsFor(selectedGroupId); var ok = true; if (conflicts.length) ok = confirm("Plan conflicts found:\n\n" + conflicts.slice(0,5).join("\n") + "\n\nSave anyway?"); if (!ok) return; var synced = saveAllState(); renderOverviewSafe(); alert(synced ? "Planned route saved and syncing." : "Planned route saved on this phone. It will sync when the top says Online sync enabled."); }
function loadNextPlanStepSafe() { if (!groups[selectedGroupId]) return; var plan = groups[selectedGroupId].routePlan || [], idx = Number(groups[selectedGroupId].planIndex || 0); if (!plan.length) { alert("No planned route yet."); return; } if (idx >= plan.length) idx = 0; var step = plan[idx], station = getStationInfo(step.stationId); if (!station) return; $("locationSelect").value = station.locationId; fillStationSelectSafe(); $("stationSelect").value = station.id; $("durationInput").value = step.duration || 15; groups[selectedGroupId].planIndex = idx + 1; saveAllState(false); checkLiveCapacitySafe(); alert("Loaded step " + (idx + 1) + ": " + station.name); }
function getPlanConflictsFor(groupId) { var group = groups[groupId], plan = group ? (group.routePlan || []) : [], conflicts = []; for (var i = 0; i < plan.length; i++) { var station = getStationInfo(plan[i].stationId); if (!station || station.capacity >= 9999) continue; var count = 1, names = []; for (var id in groups) if (Object.prototype.hasOwnProperty.call(groups, id) && id !== groupId) { var otherPlan = groups[id].routePlan || [], other = otherPlan[i]; if (other && other.stationId === station.id) { count++; names.push(groups[id].name); } } if (count > station.capacity) conflicts.push("Step " + (i+1) + " · " + station.name + " is over limit with: " + names.join(", ")); } return conflicts; }
